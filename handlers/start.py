...
<b>💡 Как записаться:</b>
1. Нажмите «✂️ Записаться» в главном меню
2. Выберите мастера
3. Выберите услугу
...
<b>📞 Если возникли вопросы:</b>
Используйте кнопку «📞 Позвонить нам» в главном меню.
"""
    await send_with_retry(message.bot, message.chat.id, text.strip(), reply_markup=keyboards.back_to_main_kb(), parse_mode="HTML")
...
async def cmd_me(message: Message):
    ...
    bookings = await storage.get_user_bookings(uid)
    loyalty = await storage.get_loyalty(uid)
    name = html.escape(message.from_user.first_name or "Гость")
    text = f"{E.USER} <b>Ваш профиль</b>\n\n"
    text += f"{E.MASTER} <b>Имя:</b> {name}\n"
    if user and user.get("phone"):
        text += f"{E.PHONE} <b>Телефон:</b> {html.escape(user['phone'])}\n"
    else:
        text += f"{E.PHONE} <b>Телефон:</b> не указан\n"
    text += "\n"
    if bookings:
        text += f"{E.CALENDAR} <b>Активные записи ({len(bookings)}):</b>\n"
        for b in bookings:
            text += f"  • {keyboards._format_date(b['date'])} {b['time']} — {html.escape(b['master'])}\n"
    else:
        text += f"{E.CALENDAR} <b>Активных записей нет</b>\n"
    text += "\n"
    if loyalty:
        visits = loyalty.get("visits", 0)
        discount = loyalty.get("discount", 0)
        text += f"{E.STAR} <b>Программа лояльности:</b>\n"
        text += f"  Визитов: {visits}"
        if discount:
...
async def cmd_master(message: Message):
    ...
    for name, info in config.MASTERS.items():
        ...
        text += f"{E.SCISSORS} <b>{html.escape(name)}</b>\n"
        if exp: text += f"  Опыт: {html.escape(exp)}\n"
        if spec: text += f"  Специализация: {html.escape(spec)}\n"
        text += "\n"
    await send_with_retry(message.bot, message.chat.id, text, reply_markup=keyboards.back_to_main_kb(), parse_mode="HTML")
...
@router.message(Command("waitlist"))
async def cmd_waitlist(message: Message):
    entries = await storage.get_user_waitlist(message.from_user.id)
    if not entries:
        await send_with_retry(message.bot, message.chat.id,
            f"{E.LIST} У вас нет активных записей в листе ожидания.",
            reply_markup=keyboards.back_to_main_kb(), parse_mode="HTML")
        return
    text = f"{E.LIST} <b>Ваш лист ожидания</b>\n\n"
    for e in entries:
        text += f"{E.CALENDAR} {keyboards._format_date(e['date'])} {e['time']}\n"
        text += f"  {E.MASTER} {html.escape(e['master'])} — {html.escape(e['service'])}\n\n"
    await send_with_retry(message.bot, message.chat.id, text, reply_markup=keyboards.back_to_main_kb(), parse_mode="HTML")
...
async def cmd_help(message: Message):
    text = (
        f"{E.INFO} <b>Справка по командам</b>\n\n"
        f"{E.LIST} <b>Основные команды:</b>\n"
        "  /start — Главное меню\n"
        "  /me — Ваш профиль и записи\n"
        "  /master — Все мастера\n"
        "  /waitlist — Лист ожидания\n"
        "  /cancel — Отменить запись\n"
        "  /help — Эта справка\n\n"
        f"{E.BARBER} <b>Как записаться:</b>\n"
        "  1. Нажмите «Записаться» в главном меню\n"
        "  2. Выберите мастера, услугу, дату, время\n"
        "  3. Введите имя и подтвердите\n\n"
        f"{E.PHONE} <b>Вопросы?</b> Кнопка «Позвонить нам» в главном меню"
    )
    await send_with_retry(message.bot, message.chat.id, text, reply_markup=keyboards.back_to_main_kb(), parse_mode="HTML")
